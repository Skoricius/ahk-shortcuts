...
class MatlabFilenameAutoComplete(sublime_plugin.EventListener):
	...
	def __get_completions(self, folder, prefix, is_object=False):
		self.depth += 1
		completions = []
		for f in os.listdir(folder):
			...
			if os.path.isfile(full_f) and ext in ('.m','.mexa64','.mexmaci64','.mexw32'): 
				...
				if ext == '.m':
					with open(full_f, 'r') as fh:
						...
						if self.fun_reg.match(func):
							...
							if is_object and params[0]=='obj':
								params = params[1:]
							if len(params)!=0 and params[-1] == 'varargin':
								params = params[:-1]
							trigger += '\t('
							content += '('
							nParam = len(params)
							for ii in range(0, nParam-1):
								...
								trigger += params[ii] + ', '
							if nParam >= 1:
								content += '${' + str(nParam) + ':' + params[nParam-1] + '}' 
								trigger += params[nParam-1]
							content += ')'
							trigger += ')'
						else:
							trigger += '\tScript'
				...
				dirname = os.path.basename(full_f)
				if dirname[0] == '@':
					completions += self.__get_completions(full_f, prefix, is_object=True)
				else:
					completions += self.__get_completions(full_f, prefix, is_object=False)
		self.depth -= 1
		return completions